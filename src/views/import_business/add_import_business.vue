    ...
    <el-form :inline="true" :model="mast_info" size="medium" label-width="100px" label-position="left" class="demo-form-inline">
      ...
      <el-form-item label="业务类型">
        <el-select filterable  v-model="mast_info.yewu_type" placeholder="请选择业务类型">
          <el-option label="统一版进口" value="统一版进口"></el-option>
          <el-option label="统一版出口" value="统一版出口"></el-option>
          <el-option label="转关" value="转关"></el-option>
        </el-select>
      </el-form-item>
      ...
      <el-form-item :label="mast_info.yewu_type == '转关'? '境内监管中转' : '境内送货'">
        <el-select filterable  v-model="mast_info.is_jingnei" placeholder="请选择境内送货">
          <el-option label="是" :value="1"></el-option>
          <el-option label="否" :value="0"></el-option>
        </el-select>
      </el-form-item>
      <el-form-item label="费用" v-if="mast_info.feiyong">
      ...
      <div class="basics_info_entering_form">
        <el-form :inline="true" :model="mast_info" label-width="96px" label-position="left" class="demo-form-inline">
          <el-form-item label="品名数量">
            <el-input :disabled="(mainTypeSelected == '' || mainTypeSelected == '一主多分' ) || mast_info.yewu_type == '统一版出口'" v-model="mast_info.descriptionNum" placeholder="请输入品名数量"></el-input>
          </el-form-item>
          <el-form-item label="报关销售代理">
            <el-input :disabled="(mainTypeSelected == '' || mainTypeSelected == '一主多分' ) || mast_info.yewu_type == '统一版出口'" v-model="mast_info.company_agency" placeholder="请输入报关销售代理"></el-input>
          </el-form-item>
          <el-form-item label="生产销售单位">
            <el-input :disabled="(mainTypeSelected == '' || mainTypeSelected == '一主多分' ) || mast_info.yewu_type == '统一版出口'" v-model="mast_info.sale_monad" placeholder="请输入生产销售单位"></el-input>
          </el-form-item>
          <el-form-item label="报关类型">
            <el-select
                :disabled="(mainTypeSelected == '' || mainTypeSelected == '一主多分' ) || mast_info.yewu_type == '统一版出口'"
          ...
          <el-form-item :label="mast_info.yewu_type == '统一版出口' ? '始发的' : '目的地'">
            <el-input :disabled="(mainTypeSelected == '' || mainTypeSelected == '一主多分' ) || mast_info.yewu_type == '统一版出口'" v-model="mast_info.destination" :placeholder=" mast_info.yewu_type == '统一版出口' ? '请输入始发的' : '请输入目的地'"></el-input>
          </el-form-item>
          <el-form-item label="主单体积">
            <el-input :disabled="(mainTypeSelected == '' || mainTypeSelected == '一主多分' ) || mast_info.yewu_type == '统一版出口'" v-model="mast_info.mast_volume" placeholder="请输入主单体积"></el-input>
          </el-form-item>
...
<!--          </div>-->
          ...
          <div class="form_item">
            <span>分单件数</span>
            <el-input :disabled="(mainTypeSelected == '' || mainTypeSelected == '直单' ) || mast_info.yewu_type == '统一版出口'" v-model="item.submenuNumPackage" placeholder="请输入分单件数"></el-input>
          </div>

          <div class="form_item">
            <span>分单毛重</span>
            <el-input :disabled="(mainTypeSelected == '' || mainTypeSelected == '直单' ) || mast_info.yewu_type == '统一版出口'" v-model="item.roughWeight" placeholder="请输入分单毛重"></el-input>
          </div>
          <!-- 品名数量-->
          <div class="form_item">
            <span>品名数量</span>
            <el-input :disabled="(mainTypeSelected == '' || mainTypeSelected == '直单' ) || mast_info.yewu_type == '统一版出口'" v-model="item.pinming_shu" placeholder="请输入品名数量"></el-input>
          </div>

          <!-- 分单计费量-->
          <div class="form_item">
            <span>报关代理</span>
            <el-input :disabled="(mainTypeSelected == '' || mainTypeSelected == '直单' ) || mast_info.yewu_type == '统一版出口'" v-model="item.baoguan_daili" placeholder="请输入报关代理"></el-input>

          </div>
          <!-- 分单计费量-->
          <div class="form_item">
            <span>生产销售单位</span>
            <el-input :disabled="(mainTypeSelected == '' || mainTypeSelected == '直单' ) || mast_info.yewu_type == '统一版出口'" v-model="item.sale_monad" placeholder="请输入生产销售单位"></el-input>
          </div>
...
<!--          </div>-->
          ...
          <div class="form_item">
            <span>报关代理</span>
            <el-select
                :disabled="(mainTypeSelected == '' || mainTypeSelected == '直单' ) || mast_info.yewu_type == '统一版出口'"
...
<!--          </div>-->
          <!-- 目的地-->
          <div class="form_item">
            <span>{{ mast_info.yewu_type == '统一版出口' ? '始发的' : '目的地' }}</span>
            <el-input :disabled="(mainTypeSelected == '' || mainTypeSelected == '直单' ) || mast_info.yewu_type == '统一版出口'"
                       v-model="item.destination" :placeholder=" mast_info.yewu_type == '统一版出口' ? '请输入始发的' : '请输入目的地'"></el-input>
          </div>
          <div class="form_item" v-if="(mainTypeSelected == '' || mainTypeSelected == '一主多分' ) || mast_info.yewu_type == '统一版出口'">
            <span>{{ mast_info.yewu_type == '转关'? '境内监管中转' : '境内送货' }}</span>
            <el-select filterable  v-model="item.is_jingnei" :disabled="(mainTypeSelected == '' || mainTypeSelected == '直单' ) || mast_info.yewu_type == '统一版出口'" placeholder="请选择境内送货">
              <el-option label="是" :value="1"></el-option>
              <el-option label="否" :value="0"></el-option>
            </el-select>
          </div>
  ...
  methods:{
    ...
    addData(){
      ...
      let data = {
        ...
        zhuanguan_number: this.mast_info.zhuanguan_number,
        feiyong: this.mast_info.feiyong,
        is_jingnei: this.mast_info.is_jingnei,
        yewu_type: this.mast_info.yewu_type,
        ...
        // TODO 报关类型
        baoguan_type:this.mast_info.baoguan_type,
        // TODO 监管类型
        jianguan_type:this.mast_info.jianguan_type,
      ...
      if(this.mast_info.busSubmenuSaveDTOS.length > 0){
        for (let item of this.mast_info.busSubmenuSaveDTOS) {
          ...
          let dto = {
            ...
            baoguan_daili: item.baoguan_daili,
            sale_monad: item.sale_monad,
