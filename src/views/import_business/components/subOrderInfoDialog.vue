<template>
  <el-dialog
      :visible.sync="dialogVisible"
      width="80%"
      :before-close="handleClose">
    <el-row style="margin-bottom: 20px">
      <el-col :span="8"> 主单号：{{ orderInfo.mainNo }}</el-col>
      <el-col :span="8"> 分单号：{{ orderInfo.submenuNo }}</el-col>
    </el-row>
    <el-form label-position="left" ref="form" :inline="true" :model="clearanceData" label-width="110px" class="order_info_list">
      <el-form-item label="分单件数:" class="info_item">
        <div class="form_value">{{ orderInfo.submenuNumPackage }}</div>
      </el-form-item>
      <el-form-item label="分单毛重:" class="info_item">
        <div class="form_value">{{ orderInfo.subRoughWeight }}</div>
      </el-form-item>
      <el-form-item label="品名数量:" class="info_item">
        <div class="form_value">{{ orderInfo.nameNum }}</div>
      </el-form-item>
      <el-form-item label="报关代理:" class="info_item">
        <div class="form_value">{{ orderInfo.customsBroker }}</div>
      </el-form-item>
      <el-form-item label="生产销售单位:" class="info_item">
        <div class="form_value">{{ orderInfo.productionSaleUnit }}</div>
      </el-form-item>
      <el-form-item label="报关类型:" class="info_item">
        <div class="form_value">{{ orderInfo.customsDeclareType == 1?'贸易代理': (orderInfo.productionSaleUnit == 2?'报关代理':'企业自理')  }}</div>
      </el-form-item>
      <el-form-item label="境内送货:" class="info_item" v-show="businessType1">
        <div class="form_value">{{ orderInfo.domesticDelivery == 1?'是':'否' }}</div>
      </el-form-item>
      <el-form-item label="费用:" class="info_item" v-if="businessType1 && orderInfo.domesticDelivery == 1">
        <div class="form_value">{{ orderInfo.expense }}</div>
      </el-form-item>
      <el-form-item label="始发港:" class="info_item" v-show="businessType1">
          <div class="form_value">{{ orderInfo.departurePort }}</div>
      </el-form-item>
      <el-form-item label="目的港:" class="info_item" v-show="businessType2 | businessType3">
        <div class="form_value">{{ orderInfo.destinationPort }}</div>
      </el-form-item>
      <el-form-item label="转关单号:" class="info_item" v-show="businessType3">
        <div class="form_value">{{ orderInfo.transitNo }}</div>
      </el-form-item>
      <el-form-item label="境内监管中转:" class="info_item" v-show="businessType3">
        <div class="form_value">{{ orderInfo.domesticSuperviseTransit == 1?'是':'否' }}</div>
      </el-form-item>
    </el-form>


    <span slot="footer" class="dialog-footer">
    <el-button type="primary" @click="dialogVisible = false">确 定</el-button>
  </span>
  </el-dialog>
</template>

<script>
export default {

  props: ["orderInfo"],
  name: "orderInfo",
  
  data(){
    return{
      dialogVisible: false,
      activeNames:"1",
      clearanceData:{
        service_shop: "",
        clearance_order:""
      },
      mainTypeShow:false,
      businessType1:false,
      businessType2:false,
      businessType3:false
    }
  },
  methods:{
    handleClose(){
      this.dialogVisible = false
    }
  },
  created() {},
  mounted() {},
  watch:{
    orderInfo(val){
    this.businessType1 = val.businessType === '统一版进口'? true:false
    this.businessType2 = val.businessType === '统一版出口'? true:false
    this.businessType3 = val.businessType === '转关'? true:false
    }
  }

}
</script>

<style scoped>
  .order_info_list{
    height: 300px;
    overflow-y: auto;
  }
  .form_value{
    width: 150px;
  }
  .info_item{
    padding-left: 10px;
    border: 1px solid #99a9bf;
  }
  .el-form-item{
    margin-bottom: 0;
    margin-right: 0;
  }

  .main-type-hidden{
    display: hidden;
  }

  .main-type-show{
    display: block;
  }

</style>
